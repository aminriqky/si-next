...
type ResponseData = pengumumanType | { error: string };

// Path to cache file
const CACHE_FILE = path.join(process.cwd(), "cache", "pengumuman.json");
...
// Read cached data
async function getCachedData(): Promise<pengumumanType | null> {
  try {
...
// Write data to cache
async function cacheData(data: pengumumanType) {
  try {
