...
type ResponseData = profilType | { error: string };

// Path to cache file
const CACHE_FILE = path.join(process.cwd(), "cache", "profil.json");
...
// Read cached data
async function getCachedData(): Promise<profilType | null> {
  try {
...
// Write data to cache
async function cacheData(data: profilType) {
  try {
