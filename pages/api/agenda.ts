...
type ResponseData = AgendaType | { error: string };

// Path to cache file
const CACHE_FILE = path.join(process.cwd(), "cache", "agenda.json");
...
// Read cached data
async function getCachedData(): Promise<AgendaType | null> {
  try {
...
// Write data to cache
async function cacheData(data: AgendaType) {
  try {
