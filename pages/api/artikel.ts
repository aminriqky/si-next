...
type ResponseData = artikelType | { error: string };

// Path to cache file
const CACHE_FILE = path.join(process.cwd(), "cache", "artikel.json");
...
// Read cached data
async function getCachedData(): Promise<artikelType | null> {
  try {
...
// Write data to cache
async function cacheData(data: artikelType) {
  try {
