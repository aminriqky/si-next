...
type ResponseData = organisasiType | { error: string };
...
// Read cached data
async function getCachedData(): Promise<organisasiType | null> {
  try {
...
// Write data to cache
async function cacheData(data: organisasiType) {
  try {
...
export async function organisasi() {
  try {
    ...
    const jsonData: organisasiType = await response.json();
    return jsonData;
  } catch (error: any) {
