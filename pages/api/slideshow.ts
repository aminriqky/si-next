...
type ResponseData = slideType | { error: string };

// Path to cache file
const CACHE_FILE = path.join(process.cwd(), "cache", "slide.json");
...
// Read cached data
async function getCachedData(): Promise<slideType | null> {
  try {
...
// Write data to cache
async function cacheData(data: slideType) {
  try {
