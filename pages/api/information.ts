...
type ResponseData = infoType | { error: string };

// Path to cache file
const CACHE_FILE = path.join(process.cwd(), "cache", "info.json");
...
// Read cached data
async function getCachedData(): Promise<infoType | null> {
  try {
...
// Write data to cache
async function cacheData(data: infoType) {
  try {
